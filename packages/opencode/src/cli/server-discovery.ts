...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const read = Effect.fn("CliServerDiscovery.read")(function* () {
      const entry = yield* fs.readJson(file).pipe(Effect.orElseSucceed(() => undefined))
    ...
    const find: Interface["find"] = Effect.fn("CliServerDiscovery.find")(function* () {
      const entry = yield* read()
    ...
    return Service.of({ write, remove, find })
  }),
)
